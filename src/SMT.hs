...
import Data.Array (Array, listArray, (!), bounds, elems, Ix)
...
import Data.Binary (Binary, encodeFile, decodeFileOrFail, encode, decode)
...
hueRGB sat tick =
  let h = (tick * 7) `mod` 360; seg = h `div` 60; f = (h `mod` 60) * 255 `div` 60; s = sat
      (r,g,b) = case seg of
        0->(255,f,0); 1->(255-f,255,0); 2->(0,255,f)
        3->(0,255-f,255); 4->(f,0,255); _->(255,0,255-f)
...
terrainSpr tick t = case t of
  ...
  Lava->lavaS tick; RockFloor->rockFloorS; Stalactite->stalactiteS
  Crystal->crystalS tick; CaveWall->caveWallS
...
overlayEntity entity terrain = zipWith (zipWith merge) entity terrain
  where merge (SCell _ (0,0,0)) t = t; merge e _ = e
...
data Combatant = Combatant
  ...
  , combMoves::[Move], combKind::Maybe DemonKind
  , combLevel::Int, combXP::Int
  } deriving (Eq,Show,Generic)
...
fromTemplateWithStats t ds = Combatant
  { combName=dName t, combHP=dMaxHP t + (dsLevel ds - 1)*8
  , combMaxHP=dMaxHP t + (dsLevel ds - 1)*8
  , combPhyAtk=dPhyAtk t + (dsLevel ds - 1)*3
  , combMagAtk=dMagAtk t + (dsLevel ds - 1)*3
  , combPhyDef=dPhyDef t, combMagDef=dMagDef t
  , combMoves=dsMoves ds, combKind=Just (dKind t)
  , combLevel=dsLevel ds, combXP=dsXP ds }
...
data WorldDemon = WorldDemon
  ...
  , wdSpawn::Pos, wdFacing::(Int,Int), wdStepCtr::Int
  } deriving (Eq,Show,Generic)
...
data BattleState = BattleState
  { bPlayer::Combatant, bAllies::[Combatant]
  , bEnemy::Combatant, bEnemyAllies::[Combatant]
  , bEnemyId::Int
  , bPhase::BattlePhase, bMenuSel::BattleMenu
  , bLog::[String]
  , hitFlash::Int, allyFlash::Int
  } deriving (Generic)
...
genCavern x y =
  ...
  in if e<250 then Lava
     else if e<400 then RockFloor
     else if e<650 then if d<8 then Crystal else if d<18 then Stalactite else RockFloor
     else CaveWall

buildSwampMap :: Array Pos Terrain
buildSwampMap = listArray ((0,0),(mapW-1,mapH-1))
  [genSwamp x y | y<-[0..mapH-1], x<-[0..mapW-1]]

buildCavernMap :: Array Pos Terrain
buildCavernMap = listArray ((0,0),(mapW-1,mapH-1))
  [genCavern x y | y<-[0..mapH-1], x<-[0..mapW-1]]

buildMaps :: Map MapId MapData
buildMaps = Map.fromList
  [ (SwampMap, MapData
...
initialState ms = GameState
  { maps=ms, currentMap=SwampMap, player=initialPlayer
  , demons=initialDemons, screen=Exploring, animTick=0
  , msgLog=["You enter the fetid swamp...","SPACE: attack · Ω: portal"]
  , nextDmnId=5, spawnTimer=0 }

savePath :: FilePath
savePath = "daemon_hunt.sav"

toSaveData :: GameState -> SaveData
toSaveData gs = SaveData
  { savePlayer=player gs, saveDemons=demons gs
  , saveCurrentMap=currentMap gs, saveNextDmnId=nextDmnId gs
  , saveSpawnTimer=spawnTimer gs, saveMsgLog=take 10 (msgLog gs) }

fromSaveData :: Map MapId MapData -> SaveData -> GameState
fromSaveData ms sd = GameState
  { maps=ms, currentMap=saveCurrentMap sd
  , player=savePlayer sd, demons=saveDemons sd
  , screen=Exploring, animTick=0
  , msgLog="(save loaded)" : saveMsgLog sd
  , nextDmnId=saveNextDmnId sd, spawnTimer=saveSpawnTimer sd }
  ...
  in if alive >= cap || spawnTimer gs < spawnThreshold
     then gs { spawnTimer = spawnTimer gs + 1 }
       ...
       let kinds = mdSpawnKinds md
           ...
           candidates = [ (x,y) | x<-[0..mapW-1], y<-[0..mapH-1]
                        , let t = mdTerrain md ! (x,y)
                        , isPassable t
                        , abs(x-fst pp)+abs(y-snd pp) > 8
                        , not (any (\wd->wdPos wd==(x,y)) (Map.elems (demons gs))) ]
...
awardXP kind xp statsMap =
  let old    = fromMaybe (defaultDemonStats kind) (Map.lookup kind statsMap)
      ...
      newMoves = nub $ dsMoves old ++
        [mv | lv <- [dsLevel old + 1 .. newLvl]
            , Just mv <- [levelUpMoves kind lv]]
      msgs   = if lvlUp
               then [ dName (demonTemplates Map.! kind) ++ " reached level " ++ show newLvl ++ "!"
                    ] ++ [ "  Learned " ++ fst3 (moveInfo mv) ++ "!"
                         | mv <- newMoves, mv `notElem` dsMoves old ]
               else []
...
awardPlayerXP xp pl =
  ...
  in (pl { playerXP=newXP, playerLevel=newLvl
         , playerMaxHP=playerMaxHP pl + bonus, playerHP=playerHP pl + bonus }, msgs)
...
boxedPanel (br,bg,bb) innerW label rows =
  let borderA = rgb br bg bb
      ...
      top = V.string borderA "╔" V.<|> V.string borderA (replicate padL '═')
         V.<|> V.string labelA label V.<|> V.string borderA (replicate padR '═')
         V.<|> V.string borderA "╗"
...
spriteImage flash spr = V.vertCat $ map renderRow spr
  where
    renderRow cells = V.horizCat $ map (\(SCell ch (r,g,b)) ->
      let (r',g',b') = flashTint flash (r,g,b) in V.char (rgb r' g' b') ch) cells

hpBarImg :: Int -> Int -> Int -> V.Image
hpBarImg tick hp maxHp =
  let pct=(hp*16)`div`max 1 maxHp; (r,g,b)=hpColour hp maxHp
...
renderExplore gs =
  ...
  where
    pl=(player gs); (cx,cy)=camOrigin(playerPos pl); tick=animTick gs
...
renderExploreSidebar gs =
  let tick=animTick gs; pl=player gs
      md=maps gs Map.! currentMap gs
      ...
      sep=V.string dimA (replicate 26 '─'); sp=V.string V.defAttr " "
      ...
      partyLines=if null (playerParty pl) then [V.string dimA "  (no demons)"]
        else zipWith (\i k ->
      ...
      ctrls=[V.string (rgb 155 155 165) "  WASD   move"
  ...
  in V.vertCat $
     ...
     ++partyLines++[sp,sep,ctrlHdr]++ctrls++[sp,sep,logHdr]++logLines
...
renderBattle gs bs = [raw $ V.vertCat [titleBar,mainArea,bottomArea]]
  where
    tick=animTick gs; flash=hitFlash bs; aflash=allyFlash bs
    md=maps gs Map.! currentMap gs; (ar,ag,ab)=mdAmbient md
    ...
    titleBar =
      let titleImg=V.horizCat $ zipWith (\i ch ->
          ...
          phaseStr=case bPhase bs of
            PlayerMenu 0->"  YOUR TURN  "; PlayerMenu i->"  SLOT "++show i++"  "
            PlayerFight _ _->"  CHOOSE MOVE  "; PlayerSummon _->"  SUMMON  "
            EnemyTurn _ _->"  ENEMY TURN  "
            BattleOver True->"  VICTORY!  "; BattleOver False->"  RETREATED  "
      in titleImg V.<|> V.string (boldA (rgb tr tg tb)) phaseStr

    mainArea=V.horizCat [enemyPanel, V.string dimA " ", playerPanel]

    enemyPanel =
      let e=bEnemy bs; kind=fromMaybe SwordDemon (combKind e); frame=tick`mod`2
          ...
          nameImg=V.horizCat $ zipWith (\i ch ->
            let (r,g,b)=hueRGB 240 (tick*2+i*30) in V.char (boldA (rgb r g b)) ch) [0..] (combName e)
          lvlImg=V.string dimA $ " Lv"++show (combLevel e)++" ["++show (combXP e)++"xp]"
          rows=[nameImg V.<|> lvlImg, hpBarImg tick (combHP e)(combMaxHP e)
               ,V.string dimA $ "PHY:"++show(combPhyAtk e)++" MAG:"++show(combMagAtk e)++" DEF:"++show(combPhyDef e)
               ,V.string V.defAttr " ", spriteImage (min 255(flash*64)) spr]
      in boxedPanel (bcr,bcg,bcb) panelW " ENEMY " rows

    playerPanel =
      let p=bPlayer bs; pFlash=min 255(aflash*64); pSprImg=spriteImage pFlash (largeSpr playerSpr)
          ...
          mkAllyRows i a =
            let aKind=fromMaybe SwordDemon (combKind a); aFrame=(tick+i)`mod`2
          ...
          allyRows=if null(bAllies bs) then [V.string dimA " (no demons summoned)"]
    ...
    menuPanel = case bPhase bs of
      PlayerMenu slot ->
        let sel=bMenuSel bs; isHayato=slot==0
            actor=maybe "???" combName (playerSlotCombatant bs slot)
            items=if isHayato
              then [(MenuFight,"FIGHT","Attack the enemy"),(MenuSummon,"SUMMON","Call a party demon")
                   ,(MenuRecruit,"RECRUIT","Recruit at low HP"),(MenuBag,"BAG","Use an item"),(MenuRun,"RUN","Flee")]
      ...
      PlayerFight slot idx ->
        let attacker=fromJust(playerSlotCombatant bs slot); moves=combMoves attacker
            rows=V.string dimA " Choose attack:" :
              zipWith(\i mv -> let (nm,dt,pw)=moveInfo mv; isSel=i==idx
                                   dtStr=case dt of Physical->"PHY";Magical->"MAG"
                               in cursor isSel V.<|> selLabel isSel nm V.<|> V.string dimA (" ["++dtStr++" "++show pw++"]")) [0..] moves
              ++[V.string dimA "   <- back"]
        in boxedPanel (80,65,115) panelW " FIGHT " rows
      PlayerSummon idx ->
        let party=playerParty (player gs)
            rows=V.string dimA " Choose demon:" :
              if null party then [V.string dimA "   (party empty)"]
              else zipWith(\i k ->
                let t=demonTemplates Map.! k; isSel=i==idx
                    stats=fromMaybe (defaultDemonStats k) (Map.lookup k (partyStats (player gs)))
                    lvlStr=" Lv"++show(dsLevel stats)
                in cursor isSel V.<|> selLabel isSel (dName t) V.<|> V.string dimA lvlStr) [0..] party
              ++[V.string dimA "   <- back"]
...
stepDemons gs =
  let tick=animTick gs; md=maps gs Map.! currentMap gs; wm=mdTerrain md
...
checkPortal gs =
  let pl=player gs; pos=playerPos pl
      md=maps gs Map.! currentMap gs
      mPortal=find (\p->portalPos p==pos) (mdPortals md)
  in case mPortal of
       Nothing -> gs
       Just portal ->
         let destMd=maps gs Map.! portalDest portal
             ...
             newDemons=Map.fromList $ zip [0..] $ take (mdSpawnCap destMd)
               [WorldDemon i k p p (1,0) 0
               |(i,(k,p))<-zip [0..] (zip (cycle (mdSpawnKinds destMd))
         ...
         in addMsg ("Entered " ++ mapName ++ "!") $
            gs { currentMap=portalDest portal
...
enemyAction slot bs = case enemySlotCombatant bs slot of
  ...
  Just e ->
    let mv=combMoves e!!(length(bLog bs)`mod`length(combMoves e))
        ...
        tgtSlot=head aliveSlots; tgt=fromJust(playerSlotCombatant bs tgtSlot)
...
tryRecruit gs bs =
  ...
  in if hpPct>30 then gs{screen=Fighting(blogMsg(combName e++" won't join yet!") bs)}
     else if length party>=4 then gs{screen=Fighting(blogMsg "Party full! (max 4)" bs)}
     else
       ...
       in addMsg msg $ gs
            { player=(player gs){playerParty=playerParty(player gs)++[kind]
...
executePlayerMove gs bs slot idx =
  let attacker=fromJust(playerSlotCombatant bs slot)
      mv=combMoves attacker!!idx; (nm,_,_)=moveInfo mv
...
executeSummon gs bs idx =
  let kind=playerParty(player gs)!!idx; tmpl=demonTemplates Map.! kind
...
tryMove (dx,dy) gs =
  let pl=player gs;(px,py)=playerPos pl; np=(px+dx,py+dy)
      md=maps gs Map.! currentMap gs
...
tryAttack gs =
  let pl=player gs;(px,py)=playerPos pl;(fx,fy)=playerFacing pl; fp=(px+fx,py+fy)
         ...
         let tmpl=demonTemplates Map.! wdKind wd
             bs=BattleState
               { bPlayer=playerCombatant{combHP=playerHP pl,combMaxHP=playerMaxHP pl
                                        ,combLevel=playerLevel pl,combXP=playerXP pl}
...
app=App{appDraw=drawGame,appChooseCursor=neverShowCursor
       ,appHandleEvent=handleEvent,appStartEvent=pure()
