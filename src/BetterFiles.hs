...
-- | A minimal, self-contained software-transactional-memory built on top of files. Each 'FileTVar' is backed by a file on disk plus an in-memory, version-stamped cache. Transactions log their reads and writes in a 'TRec'; at commit time we lock the union of the read+write set, validate that every value we read is still at the version we read it at, flush writes to disk atomically (temp -> fsync -> rename -> fsync dir), bump versions, and unlock.
-- This mirrors GHC's STM commit protocol (read log + write log + validate + commit), but with the filesystem as durable storage and fcntl/flock as the cross-process mutual-exclusion mechanism.
-- ghc -threaded -rtsopts -with-rtsopts=-N src/BetterFiles.hs -o fstm-demo && ./fstm-demo
...
-- | A staged write: the serialised bytes to flush, an action that applies the new (version, value) to the in-memory cache, and the commit lock.
...
-- | The transaction monad: plain IO with a 'TRec' threaded through. Kept simple (no transformer stack) so the mechanics stay visible.
...
-- | Create a variable. If the backing file exists, its value is loaded; otherwise the file is initialised with @initial@.
...
-- | Transactional read. If we have already staged a write to this variable, we see our own write (read-your-writes). Otherwise we read the cache and log the observed version for later validation. The original read entry is kept even if the variable is written later, so the dependency is validated.
...
-- | Transactional write. Stages the serialised value and a cache-update action. Note we do NOT remove any prior read entry: the read that informed this write must still be validated at commit (this is what prevents lost updates).
...
commit tr = do
  ...
  -- Lock the UNION of the read and write sets (deduplicated by path), in a deterministic order (sorted by path) to avoid deadlock. Locking the read set too — not just the write set — is what gives serializability and rules out write-skew, matching GHC STM's commit which acquires every TVar it touched.
...
-- | Acquire each (path, lock) in order: take the in-process mutex AND the cross-process fcntl write lock, run the action, release both. Nested so all locks are held simultaneously during validate+commit.
...
-- | write temp -> fsync temp -> rename over target -> fsync directory. rename(2) is atomic on POSIX; the directory fsync makes the rename itself survive a crash on filesystems that need it.
...
-- | Run a transaction to completion, retrying on conflict. The IORef counts conflicts so the demo can report contention.
...
-- | Move @amt@ from one account to another, but only if funds suffice. Reads both balances (logging them), then writes both. Because both reads are validated at commit, two concurrent transfers that read the same starting balance cannot both win — the loser sees a version bump and retries.
...
main = do
  ...
  forM_ [1 .. nThreads] $ \tid -> forkIO $ do
    -- in each thread, read-write 250 times
...
-- | Variant of 'atomicallyFile' that logs each conflict to a Chan instead of silently retrying, so the caller can observe contention in real time.
...
-- | Spin up two threads that each increment a shared file-backed counter @n@ times (200 by default), printing every conflict as it occurs. Usage in GHCi:   conflictDemo
conflictDemo :: IO ()
conflictDemo = do
  ...
  -- A dedicated printer thread drains the conflict log so messages appear promptly without interleaving with the worker threads' output.
  logChan <- newChan
