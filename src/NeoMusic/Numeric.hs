...
module NeoMusic.Numeric
  ( numeric, numericPage, numericHtml, numericLanes, Legend(..), legend ) where
  ...
  let used = nub (map instrument es)
      ...
      describe t =
        ...
        in (if edo then show (round divisions :: Integer) ++ "-EDO" else "ratio " ++ show (A.ratio t) ++ " per step")
           ++ ", 0 = " ++ either (const (hzText (A.f0 t))) pitchName (midiPitch t (A.Step 0))
...
numericHtml hybrid perf score = do
  ...
  pure $ "<!doctype html><html lang=\"en\"><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">"
    ...
    ++"@media print{body{background:white}main{margin:0}.system{break-inside:avoid}}</style><main><h1>"++title++"</h1>"
    ...
    ++"<section class=\"legend\">"++concatMap (\l -> "<p>"++escape l++"</p>") (take 1 (legendText info))
  ...
  where
    ...
    htmlSystem rows columns =
      "<section class=\"system\" style=\"grid-template-columns:max-content repeat("++show (length columns)++",max-content)\">"
      ++concat [ "<div class=\"lane\">"++escape name++"</div>"++concat [cell (cells !! i) | i <- columns] | (name, cells) <- rows ]
      ++"</section>"
    cell Nothing = "<span></span>"
    cell (Just e) = "<span class=\"event\">"++(if hybrid && not (null (pitches e)) then "<span class=\"bar\" aria-hidden=\"true\"></span>" else "")
    ...
    keyboard ks =
      let byMidi = M.fromList [(p, x) | (x, p) <- ks]
          ...
          key p =
            let black = isBlack p
                ...
                classes = (if black then "b" else "w") ++ maybe "" (const " u") step
      ...
      in "<div class=\"keys\"><div class=\"kb\" role=\"img\" aria-label=\"Keyboard: highlighted keys are the steps this piece uses\" style=\"width:"
         ++show (whiteCount * 30)++"px\">"++concatMap key [lo .. hi]++"</div></div>"
