...
data Instr
  ...
  | Br Int
  | BrIf Int -- de Bruijn label depth, like real wasm
     ...
     in case M.lookup res (wGrants w) of
          Just True ->
            let fd = aNextFd a
                a' =
                  a
                    { aFds = IM.insert fd (FdFile path 0) (aFds a),
                      aNextFd = fd + 1
                    }
             in Right
                  ( w,
      ...
      Runnable vm -> case step vm of
        ...
        NeedHost fn args vm' ->
          let vmCharged = vm' {vmFuel = vmFuel vm' - hostFuelCost}
           in case doHost wd act {aStatus = Runnable vmCharged} fn args of
