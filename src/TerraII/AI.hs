...
runFaction gs fid = case Map.lookup fid (gsFacs gs) of
  Nothing -> gs
...
insectOrder fid gs u =
  let a = getArea (unitLoc u) gs
      ...
      enemies = filter (isEnemyArea fid gs) (areaAdj a)
   in case free of
        (t : _) -> aiClaim fid u t (addLog' (LogEntry (gsTurn gs) (Just fid) (factionColored fid "Hiveform" ++ " spreads to " ++ areaName (getArea t gs)) LInfo) gs)
        [] -> case enemies of
          (t : _) -> aiRaid fid u t gs
          [] -> gs
...
thiefOrder fid gs u =
  let a = getArea (unitLoc u) gs
   ...
   in case unitType u of
        ...
        Laborer -> buildOutpost fid u gs
        _ -> case weak of
          (t : _) -> aiRaid fid u t (addLog' (LogEntry (gsTurn gs) (Just fid) (factionColored fid "Thieves" ++ " strike " ++ areaName (getArea t gs) ++ " under cover of dark") LCombat) gs)
          [] -> case free of
            (t : _) -> aiClaim fid u t gs
            [] -> gs
...
-- State -> State based on area id
aiMove :: Unit -> AreaId -> GameState -> GameState
aiMove u dest gs =
  let src = unitLoc u
      uid = unitId u
      rmSrc a = a {areaUnits = Map.adjust (filter (/= uid)) (unitOwner u) (areaUnits a)}
      addDst a = a {areaUnits = Map.insertWith (++) (unitOwner u) [uid] (areaUnits a), areaExplored = Set.insert (unitOwner u) (areaExplored a)}
   in gs {gsUnits = Map.adjust (\u2 -> u2 {unitLoc = dest}) uid (gsUnits gs), gsAreas = Map.adjust rmSrc src $ Map.adjust addDst dest (gsAreas gs)}
   ...
   in if wins
        then
          let win = factionColored attFid "Victory! " ++ areaName target ++ " falls to " ++ factionShort attFid ++ "!"
              ...
              gs4 = gs3 {gsAreas = Map.adjust (\a -> a {areaOwner = Just attFid, areaUnits = Map.insertWith (++) attFid [unitId u] $ maybe (areaUnits a) (\df -> Map.delete df (areaUnits a)) defFid}) targetId (gsAreas gs3)}
...
-- always returning a gamestate record, typically needs to be immutably updated to point to existing and new stuff
   ...
   in if canAfford (factionResources fac) cost && not (null myBases)
        then
          let uid = gsNextUID gs
              loc = head myBases
              newU = Unit uid ut fid (unitTypeName ut ++ " #" ++ show uid) (unitMaxHP ut) 0 Green loc False
              gs' = gs {gsUnits = Map.insert uid newU (gsUnits gs), gsNextUID = uid + 1, gsAreas = Map.adjust (\a -> a {areaUnits = Map.insertWith (++) fid [uid] (areaUnits a)}) loc (gsAreas gs), gsFacs = Map.adjust (\f -> f {factionResources = subRes (factionResources f) cost}) fid (gsFacs gs)}
           in gs'
        else gs
      ...
      if factionDefeated f
        then gs
          ...
          let myAreas = [a | a <- Map.elems (gsAreas gs), areaOwner a == Just fid]
              ...
              net = Resources {resFood = max 0 (resFood (factionResources f) + fIncome - resFood upkeep), resMat = max 0 (resMat (factionResources f) + resMat income - resMat upkeep), resFuel = max 0 (resFuel (factionResources f) + resFuel income - resFuel upkeep), resResearch = resResearch (factionResources f) + resResearch income}
   ...
   in if tl <= 0
        then
          let (dur, rng') = randomR (4 :: Int, 10) (gsRng gs)
              ...
              gs' = gs {gsRng = rng', gsEnv = Env (envNext e) dur next2, gsMsg = Just msg}
...
-- do ANSI escape for TUI colors like \ESC[0m == normal, \ESC[91m == red
...
checkVictory gs =
  let defeated = [fid | fid <- [minBound .. maxBound], not (hasBases fid gs)]
      ...
      active = [fid | fid <- [minBound .. maxBound], let f = Map.lookup fid (gsFacs gs1), maybe False (not . factionDefeated) f]
   in if length active == 1
        then
          let w = head active
              msg = factionColored w (factionShort w) ++ " has conquered Terra II!"
           in gs1 {gsOver = Just (GWin w), gsLog = LogEntry (gsTurn gs1) (Just w) msg LDefeat : gsLog gs1}
        else gs1

hasBases :: FactionId -> GameState -> Bool
hasBases fid gs = any (\a -> areaOwner a == Just fid && isJust (areaBase a)) (Map.elems (gsAreas gs))
