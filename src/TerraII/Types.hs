...
-- Factions
data FactionId = NeoRepublic | SouthernEmpire | Insects | Thieves deriving (Show, Eq, Ord, Enum, Bounded)

-- factionFgColor: ANSI color code
data Faction = Faction {factionId :: FactionId, factionName :: String, factionFgColor :: String, factionSym :: Char, factionResources :: Resources, factionTech :: Set TechId, factionMorale :: Int, factionLeader :: String, factionDefeated :: Bool} deriving (Show, Eq)

-- Resources
data Resources = Resources {resFood :: Int, resMat :: Int, resFuel :: Int, resResearch :: Int} deriving (Show, Eq)
...
canAfford have cost = resFood have >= resFood cost && resMat have >= resMat cost && resFuel have >= resFuel cost && resResearch have >= resResearch cost
...
data Tech = Tech {techId :: TechId, techName :: String, techDesc :: String, techCost :: Int, techPrereqs :: [TechId], techEffect :: String} deriving (Show, Eq)
...
-- Unit types
data UnitType = Recon | Laborer | Militia | LightInfantry | MechInfantry | HeavyInfantry deriving (Show, Eq, Ord, Enum, Bounded)

data UnitStats = UnitStats {usAtk :: Int, usDef :: Int, usFoodUp :: Int, usFuelUp :: Int, usBuildMat :: Int, usBuildFuel :: Int, usBuildFood :: Int} deriving (Show, Eq)
...
data Veterancy = Green | Regular | Veteran | Elite deriving (Show, Eq, Ord, Enum, Bounded)
...
data Unit = Unit {unitId :: UnitId, unitType :: UnitType, unitOwner :: FactionId, unitName :: String, unitHP :: Int, unitXP :: Int, unitVet :: Veterancy, unitLoc :: AreaId, unitOrdered :: Bool} deriving (Show, Eq)

-- Areas
type AreaId = Int

data AreaTerrain = Grassland | Forest | Mountains | Ruins | Volcanic | Desert | Riverland | Coast | Industrial deriving (Show, Eq, Ord)
...
-- | areaHazard: 0-5
data Area = Area {areaId :: AreaId, areaName :: String, areaTerrain :: AreaTerrain, areaAdj :: [AreaId], areaOwner :: Maybe FactionId, areaBase :: Maybe Base, areaUnits :: Map FactionId [UnitId], areaExplored :: Set FactionId, areaCapacity :: Int, areaYield :: Resources, areaHazard :: Int, areaFeatures :: [String]} deriving (Show, Eq)

data Base = Base {baseOwner :: FactionId, baseLevel :: Int, baseHP :: Int, baseMaxHP :: Int, baseDef :: Int} deriving (Show, Eq)

-- Environment
data EnvPhase = ENormal | EUVHigh | EFreeze | EAshStorm deriving (Show, Eq)
...
-- | envTurnLeft: turns until phase change
data Env = Env {envPhase :: EnvPhase, envTurnLeft :: Int, envNext :: EnvPhase} deriving (Show, Eq)

-- Log
data LogSev = LInfo | LWarn | LCombat | LDiscover | LDefeat deriving (Show, Eq)

-- Required for lookng back actions
data LogEntry = LogEntry {leTurn :: Int, leFac :: Maybe FactionId, leMsg :: String, leSev :: LogSev} deriving (Show, Eq)

-- Game state
data Phase = PlayerOrders | Resolution | SitRep deriving (Show, Eq)

data Sel = SelNone | SelUnit UnitId | SelArea AreaId deriving (Show, Eq)

data GameResult = GWin FactionId | GLose String deriving (Show, Eq)

data GameState = GameState {gsTurn :: Int, gsAreas :: Map AreaId Area, gsFacs :: Map FactionId Faction, gsUnits :: Map UnitId Unit, gsEnv :: Env, gsLog :: [LogEntry], gsNextUID :: UnitId, gsRng :: StdGen, gsPhase :: Phase, gsSel :: Sel, gsPlayer :: FactionId, gsOver :: Maybe GameResult, gsMsg :: Maybe String} deriving (Show)
...
padR n s | vlen s >= n = take n (stripA s)
padR n s = s ++ replicate (n - vlen s) ' '

padL :: Int -> String -> String
padL n s | vlen s >= n = take n (stripA s)
padL n s = replicate (n - vlen s) ' ' ++ s
