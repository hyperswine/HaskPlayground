...
header gs =
  let fac    = getFac NeoRepublic gs
      ...
      e      = gsEnv gs
      eStr   = envPhaseColor (envPhase e) ++ bold (envPhaseName (envPhase e)) ++ reset
               ++ dim (" [" ++ show (envTurnLeft e) ++ "t]")
      rStr   = col 32 ("F:" ++ show (resFood res))
            ++ "  " ++ col 33 ("M:" ++ show (resMat res))
            ++ "  " ++ col 91 ("Fu:" ++ show (resFuel res))
            ++ "  " ++ col 36 ("R:" ++ show (resResearch res))
  in [ bold (col 94 " TERRA II") ++ "  T:" ++ bold (show (gsTurn gs))
       ++ "  |  " ++ eStr
       ++ "  |  " ++ rStr
     , " Playing: " ++ factionColored NeoRepublic (factionName fac)
       ++ "  |  Leader: " ++ factionLeader fac
       ++ "  |  Morale: " ++ moraleStr (factionMorale fac)
     ]
...
body gs =
  let mapL   = renderMap gs
      ...
      combined = zipWith (\l r -> padR w l ++ " | " ++ r)
                          (padTo maxL mapL) (padTo maxL rightL)
  in combined

-- Map rendering
renderMap :: GameState -> [String]
renderMap gs =
  bold " MAP" : concatMap (renderMapRow gs) gridLayout ++ [dim " ~coast  *forest  ^mtn  Hruins  Ovolcanic  ~ river  ~desert  #industrial"]
  ...
  in case ma of
    ...
    Just a  ->
      let known   = Set.member pf (areaExplored a)
          ...
          baseMk  = if isJust (areaBase a)
                    then maybe "" (\f -> factionColored f "=") (areaOwner a)
                    else " "
      ...
      in if not known
         then [ selL ++ dim "   ???   " ++ selR
              , selL ++ "         " ++ selR
              , "             " ]
         else [ selL ++ nm6 ++ ownCol ++ baseMk ++ selR
              , selL ++ tChar ++ " " ++ yStr ++ " " ++ hazC ++ selR
              , "  " ++ unitMk ++ "          " ]
...
renderUnitMark a pf =
  let mineIds = fromMaybe [] $ Map.lookup pf (areaUnits a)
      ...
      mkFoe   = case others of
                  [] -> ""
                  _  ->
                    let fid = head $ Map.keys $ Map.filter (not . null) $
                                Map.filterWithKey (\k _ -> k /= pf) (areaUnits a)
                    in factionColored fid "!"
  in mkMine ++ mkFoe
...
overviewPanel gs =
  ...
  in [ bold " OVERVIEW"
     ...
     , " Units : " ++ bold (show (length myUs))
       ++ "  Unordered: " ++ (if null unord then col 32 "all done" else col 93 (show (length unord) ++ " waiting"))
     , ""
     , " RIVALS:"
    ...
    Just f  ->
      if factionDefeated f
        then " " ++ dim (factionShort fid ++ " [ELIMINATED]")
          ...
          in " " ++ factionColored fid (padR 8 (factionShort fid))
             ++ " areas:" ++ show as ++ " units:" ++ show us
...
areaUnitLines a gs =
  ...
  in if null uids then [dim "   No units."]
     else map (\uid -> case Map.lookup uid (gsUnits gs) of
                Nothing -> ""
                Just u  -> "   " ++ factionColored (unitOwner u) "[" ++ factionShort (unitOwner u) ++ "] "
                           ++ unitName u ++ " (" ++ unitTypeShort (unitType u) ++ ")"
                           ++ " HP:" ++ show (unitHP u)) uids
