...
import TerraII.AI (runAI, tickResources, tickEnv, checkVictory, addLog')
...
input '\ESC' gs = do
  c1 <- getChar
  if c1 == '['
    then do c2 <- getChar; arrowKey c2 gs
    else return gs { gsSel = SelNone }
input c gs = case gsSel gs of
  SelUnit uid -> unitCmd c uid gs
  _           -> return gs

arrowKey :: Char -> GameState -> IO GameState
arrowKey _ gs = return gs  -- reserved for future cursor navigation
  ...
  case Map.lookup uid (gsUnits gs) of
    ...
    Just u  ->
      let a   = getArea (unitLoc u) gs
          adj = areaAdj a
          enemies = filter (\aid -> isEnemy aid gs) adj
      in case c of
        ...
        n | n `elem` ['1'..'9'] ->
            ...
            in if idx < length adj
               then return $ doMove uid (adj !! idx) gs
               else return gs
        -- raid by R1, R2... handled via 'r' followed by digit in separate pass
        -- simplified: 'r' key raids first enemy
        _ -> return gs
  ...
  case Map.lookup uid (gsUnits gs) of
    ...
    Just u  ->
      let loc  = unitLoc u
          ...
          gs1  = gs { gsAreas = foldr (\aid acc ->
                    Map.adjust (\ar -> ar { areaExplored = Set.insert NeoRepublic (areaExplored ar) }) aid acc
                    ) (gsAreas gs) toReveal }
  ...
  case Map.lookup uid (gsUnits gs) of
    ...
    Just u  ->
      ...
      in if areaOwner a /= Just NeoRepublic
         ...
         else if isJust (areaBase a)
         then addMsg "A base already exists here." gs
         else if not (canAfford (factionResources fac) cost)
         then addMsg "Need M:20 to build a base." gs
         else
           let newBase = Base NeoRepublic 1 30 30 3
               gs1 = gs { gsAreas = Map.adjust (\ar -> ar { areaBase = Just newBase }) loc (gsAreas gs)
                         , gsFacs = Map.adjust (\f -> f { factionResources = subRes (factionResources f) cost }) NeoRepublic (gsFacs gs) }
  ...
  case Map.lookup uid (gsUnits gs) of
    ...
    Just u  ->
      let loc  = unitLoc u
          a    = getArea loc gs
          yld  = areaYield a
          gs1  = if isNothing (areaOwner a)
                 then gs { gsAreas = Map.adjust (\ar -> ar { areaOwner = Just NeoRepublic }) loc (gsAreas gs) }
                 else gs
      ...
      in if cur >= cap
         then addMsg ("Cannot move to " ++ areaName da ++ ": unit capacity reached.") gs
           ...
           let rmSrc ar = ar { areaUnits = Map.adjust (filter (/= uid)) NeoRepublic (areaUnits ar) }
               addDst ar = ar
                 { areaUnits   = Map.insertWith (++) NeoRepublic [uid] (areaUnits ar)
                 , areaExplored = Set.insert NeoRepublic (areaExplored ar)
                 , areaOwner   = if isNothing (areaOwner ar) then Just NeoRepublic else areaOwner ar }
               gs1 = gs { gsUnits = Map.adjust (\u2 -> u2 { unitLoc = dest }) uid (gsUnits gs)
      ...
      in if wins
         then
           let gs3 = addLog' (LogEntry (gsTurn gs) (Just NeoRepublic) ("Victory! " ++ areaName target ++ " captured!") LCombat) gs2
               ...
               -- take area
               gs5 = gs4 { gsAreas = Map.adjust (\a -> a
                 { areaOwner = Just NeoRepublic
                 , areaUnits = Map.insertWith (++) NeoRepublic [uid]
                             $ maybe id (\df -> Map.delete df) defFid (areaUnits a)
                 }) targetId (gsAreas gs4) }
...
researchMenu gs = do
  ...
  putStrLn ""
  forM_ (zip [1..] avail) $ \(i, t) ->
    putStrLn $ col 96 ("  [" ++ show i ++ "] ") ++ bold (techName t)
             ++ " (" ++ show (techCost t) ++ "rp)"
             ++ "\n      " ++ dim (techDesc t)
             ++ "\n      " ++ col 32 (techEffect t)
  putStrLn ""
    ...
    n | n `elem` ['1'..'9'] ->
        ...
        in if idx < length avail
           then
             ...
             in if rp >= techCost t
                then
                  let gs' = gs { gsFacs = Map.adjust (\f -> f
                        { factionTech      = Set.insert (techId t) (factionTech f)
                        , factionResources = (factionResources f) { resResearch = rp - techCost t }
                        }) NeoRepublic (gsFacs gs) }
...
baseMenu gs = do
  ...
  forM_ types $ \(ut, i) ->
    let s = unitStats ut
    in putStrLn $ "  " ++ col 96 ("[" ++ show i ++ "] ") ++ padR 16 (unitTypeName ut)
               ++ "  M:" ++ show (usBuildMat s) ++ " Fu:" ++ show (usBuildFuel s) ++ " F:" ++ show (usBuildFood s)
               ++ "  Atk:" ++ show (usAtk s) ++ " Def:" ++ show (usDef s)
  putStrLn ""
  case myBases of
             ...
             ((bid,_):_) ->
               ...
               then
                 let uid  = gsNextUID gs
                     newU = Unit uid ut NeoRepublic (unitTypeName ut ++ " #" ++ show uid)
                              (unitMaxHP ut) 0 Green bid False
                 ...
                 in if cur >= cap
                    then return $ addMsg "Base area is at unit capacity!" gs
                      ...
                      let gs' = gs { gsUnits   = Map.insert uid newU (gsUnits gs)
                                   , gsNextUID = uid + 1
                                   , gsAreas   = Map.adjust (\a -> a { areaUnits = Map.insertWith (++) NeoRepublic [uid] (areaUnits a) }) bid (gsAreas gs)
                                   , gsFacs    = Map.adjust (\f -> f { factionResources = subRes (factionResources f) cost }) NeoRepublic (gsFacs gs)
                                   }
...
nextUnit gs =
  let unordered = sortBy (comparing unitId)
                $ filter (\u -> unitOwner u == NeoRepublic && not (unitOrdered u))
                $ Map.elems (gsUnits gs)
  in case unordered of
    [] ->
      let all' = sortBy (comparing unitId)
               $ filter (\u -> unitOwner u == NeoRepublic)
               $ Map.elems (gsUnits gs)
      in case all' of
        (u:_) -> gs { gsSel = SelUnit (unitId u)
                     , gsMsg = Just "All units have orders. Press [T] to end turn." }
        []    -> gs { gsSel = SelNone
                     , gsMsg = Just "No units. Build some from [B] Base Production." }
    (u:_) -> gs { gsSel = SelUnit (unitId u) }
...
ordered uid gs = gs { gsUnits = Map.adjust (\u -> u { unitOrdered = True }) uid (gsUnits gs)
                    , gsSel   = SelNone }
...
damage uid dmg gs =
  case Map.lookup uid (gsUnits gs) of
    ...
    Just u  ->
      let hp' = unitHP u - dmg
      in if hp' <= 0
         then killUnit uid gs
         else gs { gsUnits = Map.adjust (\u2 -> u2 { unitHP = hp' }) uid (gsUnits gs) }
...
killUnit uid gs =
  case Map.lookup uid (gsUnits gs) of
    ...
    Just u  ->
      ...
      in gs' { gsUnits = Map.delete uid (gsUnits gs')
              , gsAreas = Map.adjust (\a -> a { areaUnits = Map.adjust (filter (/= uid)) (unitOwner u) (areaUnits a) })
                          (unitLoc u) (gsAreas gs') }
