...
--
-- Build:  ghc -o FprSoA FprSoA.hs && ./FprSoA
-- Or:     runghc FprSoA.hs
...
instance Foldable VList where
  -- Oldest segment last in `segments`; within each segment oldest element
  -- is at the tail. foldr over the logical sequence = foldr segs-reversed,
  -- then within each segment reversed.
  foldr f z (VList segs _) = foldr (flip (foldr f)) z segs

-- | O(1) amortised cons.
-- Prepend into the head segment when there is still room; otherwise spill
-- into a new segment of double the target size.
vcons :: a -> VList a -> VList a
vcons x (VList [] _) = VList [[x]] 2
vcons x (VList (h : t) tgt)
  | length h < tgt = VList ((x : h) : t) tgt
  | otherwise = VList ([x] : h : t) (tgt * 2)
...
-- `foldr vcons vnil [a,b,c]` = `vcons a (vcons b (vcons c vnil))`, so `a`
-- ends up as the head of the head segment. `concatMap id segments` therefore
-- yields [a, b, c] directly (head segment first, head element first).
...
class FPR_Record a where
  ...
  -- | The empty SoA (nil).
  -- Proxy is required because SoA is a non-injective type family --
  -- the compiler knows the concrete type at every call site.
...
-- | Apply a list of independent ColUpdates.
-- Each touches exactly one column; order is irrelevant when fields are
-- mutually independent (the PE guarantees this statically).
...
-- | SoA layout for List Student.
-- The compiler derives this automatically from the record definition.
...
instance FPR_Record Student where
  ...
  fromSoA (StudentSoA ns as) =
    zipWith Student (vlistToList ns) (vlistToList as)
  ...
  -- \| Column-split map.
  -- The PE analyses the lambda AST and finds that the two field updates are
  -- independent: new name depends only on old name, new age only on old age.
  -- It emits two separate column passes, each a SIMD candidate.
  --
  -- The trick: we map each column by reconstructing a minimal dummy record
  -- containing only that field's value, apply f, then project the result.
  -- The PE does this symbolically (never evaluates f); here it is explicit.
  --
  -- Note: this is ONLY correct when fields are independent. The PE verifies
  -- this statically before emitting the split.
  ...
  filterSoA p (StudentSoA ns as) =
    let pairs = zip (vlistToList ns) (vlistToList as)
        kept = filter (uncurry (\n a -> p (Student n a))) pairs
        (ns', as') = unzip kept
     in StudentSoA (fromListV ns') (fromListV as')
...
-- | Build an SoA from a list literal.
-- Simulates:  students = [alice, bob]
-- desugaring: alice :: bob :: nil
...
-- Lowers to three independent SIMD vector adds in the real runtime.
...
main = do
  ...
  let alice = Student "Alice" 20
      bob = Student "Bob" 20
      charlie = Student "Charlie" 22

  -- students : List Student.
  -- students = [alice, bob].
  -- desugars to: alice :: bob :: nil
  ...
  sep
  -- f = List.map (s => {name = "Dr "++name, age = age+1}) students
  -- PE splits into two independent column maps:
  --   mapColumn nameField ("Dr " ++)
  --   mapColumn ageField  (+1)
  ...
  putStrLn $
    if (fromSoA promoted :: [Student]) == fromSoA naivePromoted
      then "  PASS"
      else "  FAIL"
