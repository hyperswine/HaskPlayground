...
mousePosBehavior = Behavior $ \t -> let t' = fromIntegral t / 1000.0 :: Double in Vec2 (300 + 200 * sin t') (200 + 100 * cos (t' * 1.3))

-- -----------------------------------------------------------------------------
-- Input Domain
-- -----------------------------------------------------------------------------

data Key = KeyW | KeyA | KeyS | KeyD | KeySpace | KeyShift deriving (Eq, Ord, Show)

data HardwareEvent = MouseMoved Vec2 | KeyDown Key | KeyUp Key deriving (Show)

data InputBuffer = InputBuffer {ibMousePos :: Vec2, ibKeyState :: Map Key Bool, ibKeyEdges :: [(Key, Bool)]}

emptyBuffer = InputBuffer (Vec2 0 0) Map.empty []

data InputSnapshot = InputSnapshot {snMousePos :: Vec2, snKeyState :: Map Key Bool, snKeyEdges :: [(Key, Bool)]}
...
data GameState = GameState {gsPlayerPos :: Vec2, gsPlayerVel :: Vec2, gsScore :: Int, gsTickCount :: Int, gsLastEdges :: [(Key, Bool)], gsMouseSample :: Vec2}

initGameState = GameState {gsPlayerPos = Vec2 400 300, gsPlayerVel = Vec2 0 0, gsScore = 0, gsTickCount = 0, gsLastEdges = [], gsMouseSample = Vec2 0 0}

-- Pure MVU update -- no IO, deterministic
update :: GameState -> Dt -> InputSnapshot -> Vec2 -> GameState
update gs dt sn mouseSample =
  let dtSec = fromIntegral dt / 1000.0 :: Double
      ax = (if keyHeld KeyD sn then 200 else 0) - (if keyHeld KeyA sn then 200 else 0)
      ay = (if keyHeld KeyS sn then 200 else 0) - (if keyHeld KeyW sn then 200 else 0)
   ...
   in gs {gsPlayerPos = newPos, gsPlayerVel = newVel, gsScore = gsScore gs + (if jumped then 10 else 0), gsTickCount = gsTickCount gs + 1, gsLastEdges = snKeyEdges sn, gsMouseSample = mouseSample}
    ...
    go buf = do
      ...
      unless q $ do
        ...
        buf'' <- case mTick of
          Nothing -> return buf'
          Just () -> do
            let snap = InputSnapshot {snMousePos = ibMousePos buf', snKeyState = ibKeyState buf', snKeyEdges = ibKeyEdges buf'}
...
applyEvent (KeyDown k) buf = buf {ibKeyState = Map.insert k True (ibKeyState buf), ibKeyEdges = ibKeyEdges buf ++ [(k, True)]}
applyEvent (KeyUp k) buf = buf {ibKeyState = Map.insert k False (ibKeyState buf), ibKeyEdges = ibKeyEdges buf ++ [(k, False)]}
  ...
  where
    go n = do
      ...
      unless q $ do
        ...
        let pos = Vec2 (300 + 200 * sin (fromIntegral t / 1000.0)) (200 + 100 * cos (fromIntegral t / 1000.0 * 1.3))
...
gameActor :: TickSignal -> SnapshotReady -> RenderQueue -> QuitFlag -> IORef Time -> IO ()
...
renderFrame _t gs frameN = do
  ...
  putStrLn $ modeStr ++ "tick=" ++ pad 4 (gsTickCount gs) ++ "  pos=" ++ padV (gsPlayerPos gs) ++ "  vel=" ++ velStr ++ "  score=" ++ show (gsScore gs) ++ "  mouse=" ++ show (gsMouseSample gs) ++ edgeStr
...
fmtVel (Vec2 x y)
  ...
  where
    signed d = (if d >= 0 then "+" else "") ++ show (round d :: Int)

pad :: Int -> Int -> String
pad n x = let s = show x in replicate (n - length s) ' ' ++ s

padV :: Vec2 -> String
padV v@(Vec2 x y) = let s = show v in s ++ replicate (max 0 (12 - length s)) ' '
