...
data Light = Red | Green | Yellow deriving (Eq, Show, Ord, Enum, Bounded)

-- | A snapshot of the traffic light: which light is active and how many ticks remain before it switches.
data TrafficLight = TrafficLight {tlLight :: Light, tlTimer :: Int} deriving (Eq, Show)
...
-- | Advance the traffic light by one tick. When the timer reaches 1 the light switches immediately and the new timer is loaded with the successor light's full duration.
step :: TrafficLight -> TrafficLight
step (TrafficLight l t)
  | t <= 1 = let l' = nextLight l in TrafficLight l' (duration l')
