...
renderManifest m =
  mName m
    ...
    ++ "|"
    ++ concatMap
      (\r -> reqService r ++ ":" ++ show (reqTier r) ++ ":" ++ reqApi r ++ ";")
      (mRequires m)
...
runQa vfs pinnedHash app Dialogue {tickCompulsory, tickOptional} = do
  ...
  say $
    "  System.app: dialogue -> compulsory "
      ++ box tickCompulsory
      ++ " "
  ...
  if qaHash app /= pinnedHash
    then pure (NotLaunched "hash mismatch: app/manifest differs from pinned identity")
      ...
      if not tickCompulsory
        then
          pure
            ( NotLaunched
                ( "compulsory capabilities not granted: "
                    ++ show (map reqService comp)
                )
            )
        else do
          ...
          case sequence compHs of
            ...
            Right [netH] -> do
              ...
              camG <-
                if tickOptional
                  then
                    forM opts (mint vfs live . reqService) >>= \case
                      [Right h] ->
                        say "  System.app: optional handle minted"
                          >> pure (Granted h)
                      _ -> pure Denied
                  else
                    say "  System.app: optional denied — no channel minted"
                      >> pure Denied
