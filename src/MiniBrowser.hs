...
pAttrs s = case dropWhile isSpace s of
  ...
  r ->
    let (name, r0) = span (\c -> isAlphaNum c || c `elem` "-_:.@") r
     in if null name
          then ([], r)
          else case dropWhile isSpace r0 of
              ...
              (q : r2)
                | q == '"' || q == '\'' ->
                    let (v, r3) = break (== q) r2
                     ...
                     in ((name, v) : rest, r4)
              r2 ->
                let (v, r3) = break (\c -> isSpace c || c == '>') r2
                    (rest, r4) = pAttrs r3
                 in ((name, v) : rest, r4)
...
applyClass c st = case c of
  ...
  _
    | Just n <- num "p-" -> Just st {sPadX = sx n, sPadY = sy n}
  ...
  where
    num p
      | p `isPrefixOf` c,
        let d = drop (length p) c,
        all isDigit d,
        not (null d) =
          Just (read d)
      | otherwise = Nothing
    shade s
      | "bg-white" `isPrefixOf` s = ' '
...
scanDirectiveWarnings (El _ as ks) =
  [ "dom: unsupported directive \"" ++ k ++ "\" (ignored)"
    | (k, _) <- as,
      ("x-" `isPrefixOf` k || "ws-" `isPrefixOf` k),
...
layoutContent st availC (cx, cy) kids
  | all isText kids =
      let s = collapseWs (unwords [t | Tx t <- kids])
          ...
          place i l =
            let off = if sCenter st then max 0 ((availC - length l) `div` 2) else 0
...
diffFrames a b =
  let h = max (length a) (length b)
      ...
      (a', b') = (pad a, pad b)
      changed =
        [ (x, y) | (y, (ra, rb)) <- zip [0 ..] (zip a' b'), (x, (ca, cb)) <- zip [0 ..] (zip ra rb), ca /= cb
        ]
   in case changed of
        [] -> Nothing
        cs ->
          Just
            ( (minimum (map fst cs), maximum (map fst cs)),
...
lexMS (c : cs)
  | isSpace c = lexMS cs
  | isDigit c =
      let (d, r) = span (\x -> isDigit x || x == '.') (c : cs)
...
evalExprStr sc src = case pExpr (lexMS src) of
  Just (e, []) -> evalE sc e
  _ -> do
    putStrLn ("  ⚠ miniscript: parse error in \"" ++ src ++ "\"")
    pure (VS "<error>")
      ...
      here <- case attr "x-data" as of
        Just src -> case pObj (lexMS src) of
          Just (kvs, []) -> do
            ...
            pure [(p, r)]
          _ -> do
            putStrLn ("  ⚠ alpine: bad x-data \"" ++ src ++ "\"")
            pure []
        Nothing -> pure []
