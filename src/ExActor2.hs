...
heapAlloc v h =
  let ref = heapNextRef h
   in (ref, h {heapCells = Map.insert ref (v, 1) (heapCells h), heapNextRef = ref + 1})
...
-- Actor Registry: shared across all actors in process. Actor Iso Map: shared across all actors in process
...
eval env = \case
  ...
  Lam params body -> return $ VFn "<lambda>" env params body
  Let x e body -> do
    v <- eval env e
  ...
  Spawn hint fnExpr argExprs -> do
    ...
    st <- getActorState
    case fnv of
      VFn _ closedEnv params body -> actorSpawn st closedEnv params body args hint
...
primPrintln [v] = liftIO (print v) >> return VUnit
...
main = do
  ...
  putStrLn "\n── Example 5: fix combinator (factorial) ──"
