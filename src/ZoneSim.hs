  ...
  where
    ...
    go i t (r1:r2:rs)
      ...
      | otherwise =
          let src  = bits r1
              dst  = case pat of
                       Uniform       -> let d = bits r2
                                        in if d == src then (d+1) `mod` nZones else d
                       Hotspot z     -> z
                       NeighbourOnly -> (src + 1) `mod` nZones
              jit  = meanGapNs * (0.5 + fromIntegral ((r2 `div` 256) `mod` 1000) / 1000)
              t'   = t + jit
          in if src == dst then go i t' rs
             else Pkt src dst t mtuBytes : go (i+1) t' rs
    go _ _ _ = []
...
main = do
  ...
  hr "0. CONFIGURATION"
  printf "  ring zones            %d  (max %d hops, shortest-path)\n"
         nZones (maxHops p)
  printf "  harts / zone          %d   -> %d harts total\n"
         hartsPerZone (nZones * hartsPerZone)
  printf "  fabric clock          %.0f MHz  (%.0f ns / cycle)\n"
         fabricMHz (1000 / fabricMHz)
  printf "  HyperRAM              %.0f MB/s x %d dies/zone = %.0f MB/s zone aggregate\n"
         hyperBWmbs hyperDies (hyperBWmbs * fromIntegral hyperDies)
  printf "  ring link             %.0f MB/s raw, %.0f MB/s effective, %.0f ns/hop\n"
         linkRawMBs linkEffMBs hopNs
  printf "  SD per zone           %.0f MB/s, %.0f us first-data\n" sdBWmbs sdFirstUs
  printf "  ring MTU              %.0f B  (%.0f ns serialisation)\n"
         mtuBytes (serNs mtuBytes linkEffMBs)
  ...
  mapM_ (\b -> printf "  %-14s %10.0f %10.0f %10.0f\n"
                 (show (round b :: Int) ++ " B read")
                 (readNs p 0 b) (readNs p 1 b) (readNs p 2 b))
        [64, 256, 1024, 4096]
  printf "  %-14s %10.0f %10.0f %10.0f\n" "64 B message"
         (msgNs p 0 64) (msgNs p 1 64) (msgNs p 2 64)
  putStrLn ""
  printf "  remote/local penalty for a 64 B read: %.2fx (1 hop), %.2fx (2 hops)\n"
         (readNs p 1 64 / readNs p 0 64) (readNs p 2 64 / readNs p 0 64)
  printf "  remote/local penalty for a 4 KB read: %.2fx (1 hop), %.2fx (2 hops)\n"
         (readNs p 1 4096 / readNs p 0 4096) (readNs p 2 4096 / readNs p 0 4096)
  ...
  mapM_ (\b -> printf "  %-12s %10.0f %10.0f %8.2fx\n"
                 (humanBytes b) (readNs p 0 b) (readNs p 2 b)
                 (readNs p 2 b / readNs p 0 b))
        [64, 256, 1024, 4096, 16384, 65536, 262144, 1048576]
  printf "\n  asymptotic ratio = %.2fx  (link %.0f MB/s vs die %.0f MB/s)\n"
         (hyperBWmbs / min hyperBWmbs linkEffMBs) linkEffMBs hyperBWmbs
  ...
  printf "     bandwidth tax. Remote *small* access is what hurts (%.2fx on 64 B).\n"
  ...
  mapM_ (\m -> let p' = p { mtuBytes = m }
               in printf "  %-8s %12.0f %14.0f %16.0f\n"
                    (humanBytes m) (serNs m linkEffMBs)
                    (perHopWorstNs p') (readBoundNs p' 2 64))
        [64, 128, 256, 512, 1024, 4096]
  putStrLn ""
  printf "  At the configured %.0f B MTU: uncontended %.0f ns, bounded worst %.0f ns (%.1fx).\n"
         mtuBytes (readNs p 2 64) (readBoundNs p 2 64)
  ...
  mapM_ (\pat -> do
          ...
          printf "    %-10s %10s %10s %10s %12s\n"
                 "offered/zone" "mean" "p99" "drift" "state"
          mapM_ (\load -> do
                   let ls = simulate p (genTraffic p pat load 20000)
                       (m, _, p99, _) = stats ls
                       d = drift ls
                       st = if d > 1.5 then "SATURATED" else "stable" :: String
                   printf "    %-10s %10.0f %10.0f %9.1fx %12s\n"
                          (show (round load :: Int) ++ " MB/s") m p99 d st)
                [25, 50, 100, 150, 200, 250, 300, 400]
          putStrLn "")
        [Uniform, Hotspot 2, NeighbourOnly]
  printf "  analytic worst-case bound for reference: %.0f ns\n"
  ...
  printf "  %-10s %12s %14s %14s %10s\n"
         "size" "local us" "striped(1cons)" "striped(Ncons)" "speedup"
  mapM_ (\mb -> printf "  %-10s %12.0f %14.0f %14.0f %9.2fx\n"
                  (humanBytes (mb * 1048576))
                  (loadLocalUs p mb) (loadStriped1Us p mb) (loadStripedNUs p mb)
                  (loadLocalUs p mb / loadStriped1Us p mb))
        [0.064, 0.5, 4, 16, 64, 128]
  putStrLn ""
  printf "  Striping reaches 2x above roughly %s, and asymptotes near %.1fx.\n"
         (crossover p) (loadLocalUs p 1024 / loadStriped1Us p 1024)
  ...
  printf "  %-14s %10s %10s %14s\n"
         "actor footprint" "1 hop us" "2 hop us" "as % quantum"
  mapM_ (\kb -> printf "  %-14s %10.1f %10.1f %13.1f%%\n"
                  (show (round kb :: Int) ++ " KB")
                  (migrateUs p 1 kb) (migrateUs p 2 kb)
  ...
  printf "  worst-case remote 64B read  %8.0f ns   (2 hops, %s MTU, credited)\n"
         (readBoundNs p 2 64) (humanBytes mtuBytes)
  printf "  ring bisection bandwidth    %8.0f MB/s (2 links cut)\n" (2 * linkEffMBs)
  printf "  total DRAM bandwidth        %8.0f MB/s (%d zones)\n"
         (fromIntegral nZones * hyperBWmbs * fromIntegral hyperDies) nZones
  printf "  total SD bandwidth          %8.0f MB/s (%d cards)\n"
         (fromIntegral nZones * sdBWmbs) nZones
  printf "  compute:bisection ratio     %8.2f  (DRAM BW / bisection BW)\n"
         (fromIntegral nZones * hyperBWmbs * fromIntegral hyperDies / (2 * linkEffMBs))
  let ratio = fromIntegral nZones * hyperBWmbs * fromIntegral hyperDies
                / (2 * linkEffMBs)
  putStrLn ""
...
crossover p =
  case [ mb | mb <- [0.002, 0.004 .. 40]
             , loadLocalUs p mb / loadStriped1Us p mb >= 2.0 ] of
