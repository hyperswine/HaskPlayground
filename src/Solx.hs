...
prim _ "-" [VInt a, VInt b] = pure (VInt (a - b))
prim _ "*" [VInt a, VInt b] = pure (VInt (a * b))
prim _ "/" [VInt a, VInt b] = if b == 0 then errorWithoutStackTrace "sol: division by zero" else pure (VInt (a `div` b))
    ...
    Right tops -> do
      ...
      if not (null linErrs)
        then do
          ...
          putStrLn ""
          let interesting = [n | n <- M.keys prog, n `notElem` M.keys builtinCons]
