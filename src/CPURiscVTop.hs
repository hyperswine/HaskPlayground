...
uartRxStep s@UartRxSt{..} rxPin =
  let sampled = rxSync2
      ...
      (fsm', cnt', bitIdx', shift', valid') = case rxFSM of
        RxIdle  -> if sampled == 0
                     then (RxStart, 0, 0, rxShift, False)
                     else (RxIdle,  0, 0, rxShift, False)
        RxStart -> if rxCnt == half - 1
                     then if sampled == 0
                            then (RxData, 0, 0, rxShift, False)
                            else (RxIdle, 0, 0, rxShift, False)
                     else (RxStart, rxCnt + 1, rxBitIdx, rxShift, False)
        RxData  -> if rxCnt == clksPB - 1
                     then let shifted = pack sampled ++# slice d7 d1 rxShift
                          in if rxBitIdx == 7
                               then (RxStop, 0, rxBitIdx, shifted, False)
                               else (RxData, 0, rxBitIdx + 1, shifted, False)
                     else (RxData, rxCnt + 1, rxBitIdx, rxShift, False)
        RxStop  -> if rxCnt == clksPB - 1
                     then (RxIdle, 0, 0, rxShift, sampled == 1)
                     else (RxStop, rxCnt + 1, rxBitIdx, rxShift, False)
      dataOut = if valid' then shift' else rxData
      s' = s { rxFSM = fsm', rxCnt = cnt', rxBitIdx = bitIdx'
             , rxShift = shift', rxSync1 = rxPin, rxSync2 = rxSync1
             , rxData = dataOut, rxValid = valid' }
  in (s', (dataOut, valid'))
...
uartTxStep s@UartTxSt{..} (dat, send) =
  let clksPB = snatToNum (SNat :: SNat ClksPerBitRV) :: Unsigned 9
      ...
      (fsm', cnt', bitIdx', shift', pin') = case txFSM of
        TxIdle  -> if send && not busy
                     then (TxStart, 0, 0, dat, 1)
                     else (TxIdle, txCnt, txBitIdx, txShift, 1)
        TxStart -> if txCnt == clksPB - 1
                     then (TxData, 0, 0, txShift, 0)
                     else (TxStart, txCnt + 1, txBitIdx, txShift, 0)
        TxData  -> let bit0    = lsb txShift
                       shifted = 0 ++# slice d7 d1 txShift
                   in if txCnt == clksPB - 1
                        then if txBitIdx == 7
                               then (TxStop, 0, txBitIdx, shifted, bit0)
                               else (TxData, 0, txBitIdx + 1, shifted, bit0)
                        else (TxData, txCnt + 1, txBitIdx, txShift, bit0)
        TxStop  -> if txCnt == clksPB - 1
                     then (TxIdle, 0, txBitIdx, txShift, 1)
                     else (TxStop, txCnt + 1, txBitIdx, txShift, 1)
      s' = s { txFSM = fsm', txCnt = cnt', txBitIdx = bitIdx'
             , txShift = shift', txPin = pin' }
  in (s', (pin', busy))
...
data TopStateRV = TopStateRV
  { ctrlFSM     :: ProgFSM
  , progByteIdx :: Unsigned 12    -- 0..4095
  , progWordAcc :: BitVector 32   -- accumulates 4 bytes into one word
  , cpuRst      :: Bool
...
data SysStateRV = SysStateRV
  { sTop  :: TopStateRV
  , sCpu  :: CpuStateRV
  , sMC   :: MemCtrl          -- data RAM + UART peripheral state for CPU
  , sFifo :: FifoSt
  , sRx   :: UartRxSt
  , sTx   :: UartTxSt
  } deriving (Generic, NFDataX, Show)
...
sysStepRV SysStateRV{..} (rxPin, bramOut) =
  let top  = sTop
      ...
      (cpu1, mc1) =
        if cpuRst top
          then (initCpuStateRV, mc0)
            ...
            let (cpu', mc', _pc, _rd, _val, _wbEn) =
                  stepCpuRV cpu0 (bramOut, mc0, cpuEn)
            in (cpu', mc')
      ...
      (fifo1, txDat, txSend) =
        if not (fifoEmpty fifo) && not txBusy
          then let (f', b) = fifoPop fifo in (f', b, True)
          else (fifo, 0, False)
      (tx', _) = uartTxStep tx (txDat, txSend)

      -- ── Drain outPending into TX FIFO ───────────────────────────────
      (fifo2, top1) =
        if outPending top && not (fifoFull fifo1)
          then (fifoPush fifo1 (outPendDat top), top { outPending = False })
          else (fifo1, top)
      ...
      (top2, fifo3, cpu2, instrWrCmd) = case ctrlFSM top1 of
        ...
        PgIdle ->
          let top' | not rxVld       = top1 { cpuRst = True }
                   | rxByte == 0x50  = top1 { ctrlFSM = PgLoad
                                            , progByteIdx = 0
                                            , progWordAcc = 0
                                            , cpuRst = True }
                   | rxByte == 0x52  = top1 { ctrlFSM = PgRun
                                            , cpuRst = False
                                            , runTimeout = 0 }
        ...
        PgLoad ->
          if rxVld
            then
              ...
              in if done
                   then let f' = if not (fifoFull fifo2) then fifoPush fifo2 0x4B else fifo2
                        in (top1 { ctrlFSM = PgIdle, progByteIdx = 0, progWordAcc = 0 }
                           , f', cpu1, pgWr)
                   else (top1 { progByteIdx = byteIdx', progWordAcc = wacc'' }
                        , fifo2, cpu1, pgWr)
        ...
        PgRun ->
          let top1'  = top1 { cpuRst = False }
              ...
              (fifo2', top2') = case cpuTxMaybe of
                Just b  ->
                  if not (fifoFull fifo2)
                    then (fifoPush fifo2 b, top1' { runTimeout = 0 })
                    else (fifo2, top1' { outPending = True, outPendDat = b, runTimeout = 0 })
                Nothing ->
                  if cpuEn && not (outPending top1')
                    then (fifo2, top1' { runTimeout = runTimeout top1' + 1 })
                    else (fifo2, top1')
              -- Check halt / timeout
              cpuHalted = rvHalt cpu1
              top2'' =
                if (cpuHalted || runTimeout top2' == 0xFFFF) && not (outPending top2')
                  then top2' { ctrlFSM = PgSendDone }
                  else top2'
          in (top2'', fifo2', cpu1, Nothing)

        PgSendDone ->
          if not (fifoFull fifo2)
            then (top1 { ctrlFSM = PgIdle }, fifoPush fifo2 0x44, cpu1, Nothing)
            else (top1, fifo2, cpu1, Nothing)

      -- ── Post-cycle: enqueue 'K' ack on reset command ─────────────────
      fifo4 =
        if rxVld && rxByte == 0x58 && ctrlFSM top == PgIdle && not (fifoFull fifo3)
...
topEntityRV
  :: Clock Dom27
  -> Signal Dom27 Bit          -- uart_rx_pin
  -> Signal Dom27 (Bit, BitVector 6)  -- (uart_tx_pin, led[5:0])
