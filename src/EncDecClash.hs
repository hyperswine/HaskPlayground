...
data HTree
  = Leaf   { weight :: P.Int, symbol :: Token }
  | Branch { weight :: P.Int, left :: HTree, right :: HTree }
  deriving (P.Show)
...
buildTree fm
  ...
  | P.otherwise =
      let leaves = sortBy (comparing weight)
                 $ P.map (\(w,c) -> Leaf c w) (Map.toList fm)
      in P.Just $ mergeForest leaves
  where
    mergeForest [t]        = t
...
buildCodeTable tree =
  let codes    = buildCodes tree
      ...
      mkEntry sym = case Map.lookup sym codes of
        P.Nothing   -> (0, 0)
        P.Just bits ->
          let bv = foldl' (\acc b -> (acc `shiftL` 1) .|. if b then 1 else 0)
                         (0 :: P.Int) bits
          in  (fromIntegral bv, fromIntegral (P.length bits))
      pairs = P.take (snatToNum (SNat @MaxSyms))
            $ P.zip [0..] (P.map mkEntry leafList)
...
buildTreeROM vocab tree = go tree 0 1 (repeat (P.True, 0, 0, 0))
  where
    symIdx w = fromIntegral $
      fromMaybe 0 (P.lookup w (P.zip (P.map P.fst vocab) [0..]))

    go (Leaf _ sym) idx _ v =
      replace (fromIntegral idx :: NodeIdx) (P.True, symIdx sym, 0, 0) v
    go (Branch _ l r) idx nextFree v =
      let lIdx  = nextFree
          lSize = nodeCount l
          rIdx  = nextFree P.+ lSize
          v1 = replace (fromIntegral idx :: NodeIdx)
                       (P.False, 0, fromIntegral lIdx, fromIntegral rIdx) v
...
encStep
  :: Vec MaxSyms CodeEntry
  -> EncState
  -> (P.Maybe (Index MaxSyms), Bool)   -- (symbol input, flush)
  -> (EncState, P.Maybe (Unsigned 8))
    ...
    (acc1, fill1) = case inp of
      ...
      P.Just sym ->
        let (codeBits, codeLen) = codeROM !! sym
            shifted = (encAcc st `shiftL` fromIntegral codeLen)
...
encoder
  ...
  => Vec MaxSyms CodeEntry
  -> Signal dom (P.Maybe (Index MaxSyms))  -- symbol stream
  -> Signal dom Bool                        -- flush strobe
  -> Signal dom (P.Maybe (Unsigned 8))
...
compressor
  :: HiddenClockResetEnable dom
  => Vec MaxSyms CodeEntry
  -> Signal dom (P.Maybe (Index MaxSyms))
  -> Signal dom Bool
  -> Signal dom (P.Maybe (Unsigned 8))
compressor = encoder
...
data SerState = SerState
  { serBuf  :: Unsigned 8  -- byte currently being serialised
  , serBits :: Unsigned 4  -- remaining valid bits (0 = idle)
  } deriving (P.Show, Generic, NFDataX)
...
data DecState = DecState
  { decNode :: NodeIdx  -- current position in the tree ROM (root = 0)
  } deriving (P.Show, Generic, NFDataX)
...
decStep
  :: Vec MaxNodes TreeNode
  -> DecState
  -> (Bit, Bool)
  -> (DecState, P.Maybe (Index MaxSyms))
decStep treeROM st (b, valid)
  ...
  | P.otherwise =
      ...
      in  if isLeaf
            then (decInit, P.Just sym)           -- leaf reached: emit, reset
            else (DecState nextNode, P.Nothing)  -- still traversing
...
roundTrip
  ...
  -> Vec MaxNodes TreeNode
  -> Signal dom (P.Maybe (Index MaxSyms))
  -> Signal dom Bool   -- flush strobe (assert once after last symbol)
  -> Signal dom (P.Maybe (Index MaxSyms))
...
simEncode syms =
  let n      = P.length syms
      ...
      raw    = P.take (n P.+ 16) $
                 simulate @System
                   (\sf -> encoder demoCodeTable (fmap P.fst sf) (fmap P.snd sf))
                   (P.zip symIn flush)
  in  [ b | P.Just b <- raw ]
...
simRoundTrip syms =
  let n      = P.length syms
      ...
      symIn  = P.concatMap (\s -> P.Just s : P.replicate 8 P.Nothing) syms
                P.++ [P.Nothing] P.++ P.repeat P.Nothing
      flush  = P.replicate (n P.* 9) P.False
                P.++ [P.True] P.++ P.repeat P.False
      total  = n P.* 9 P.+ 80   -- generous window
      raw    = P.take total $
                 simulate @System
                   (\sf -> roundTrip demoCodeTable demoTreeROM
                             (fmap P.fst sf) (fmap P.snd sf))
                   (P.zip symIn flush)
...
tokenToIndex tok = fromIntegral P.<$>
  P.lookup tok (P.zip (P.map P.fst demoVocab) [(0 :: P.Int)..])
...
encodeHeader tree nBits =
  ...
  in  word32BE nWords
   P.++ P.concatMap encodeEntry vocab
   P.++ word32BE nBits
  where
    encodeEntry (w, freq) =
      let bs = P.map (fromIntegral . fromEnum) w :: [Word8]
      in  word32BE (P.length bs) P.++ bs P.++ word32BE freq
...
compress text =
  let tokens    = tokenise text
      ...
      bodyBits  = P.concatMap
                    (\t -> fromMaybe (P.error $ "OOV: " P.++ t) (Map.lookup t codes))
                    tokens
...
printTree = go 0
  where
    go depth (Leaf w sym) =
      P.putStrLn $ P.replicate (depth P.* 2) ' '
                P.++ "Leaf(" P.++ P.show w P.++ ") " P.++ P.show sym
    go depth (Branch w l r) = do
      P.putStrLn $ P.replicate (depth P.* 2) ' '
                P.++ "Branch(" P.++ P.show w P.++ ")"
      go (depth P.+ 1) l
      go (depth P.+ 1) r
...
printCodes cm =
  P.mapM_ (\(w, code) ->
    P.putStrLn $ P.show w P.++ "\t"
              P.++ P.map (\b -> if b then '1' else '0') code)
  $ sortBy (comparing (P.length . P.snd))
  $ Map.toList cm
