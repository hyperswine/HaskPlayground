...
module FPLStuff.TypeChecking where
...
data Ext = NegInf | Fin Integer | PosInf
  deriving (Eq, Ord)
...
-- Fully conservative division: we make no attempt to be clever.  This is
-- exactly the "won't go further than the domains we defined" boundary --
-- div simply loses all information.  (A logHalf-style function is the
-- documented false-rejection class.)
iDiv :: Itv -> Itv -> Itv
iDiv Bot _ = Bot
iDiv _ Bot = Bot
iDiv _ _   = top
...
data Expr
  ...
  | Fold Int Expr Expr String Expr
      -- Fold id lo hi f acc: builtin bounded recursion scheme.
      -- iterates i from lo..hi calling f i acc; iteration count is
      -- statically bounded by (hi - lo + 1).  The Int is a site id for
      -- the implicit calls to f.
  deriving (Eq, Show)
...
callsIn e = case e of
  ...
  Fold i lo hi f acc ->
    (i, f, []) : callsIn lo ++ callsIn hi ++ callsIn acc
...
findMeasure fd =
  let selfs = allSelfCalls (fName fd) (fBody fd)
      ...
      decreasesAt idx (Param nm pre) =
        let steps = [ k | (_, as) <- selfs
                        , length as == length ps
                        , Bin Sub (Var x) (Lit k) <- [as !! idx]
                        , x == nm, k >= 1 ]
...
checkSafety env fd
  ...
  | otherwise = do
      ...
      case [ g | g <- S.toList reach
               , Just gd <- [M.lookup g env], fSafety gd == UnsafeFn ] of
        (g:_) -> Left (ESafeCallsUnsafe (fName fd) g)
        []    -> Right ()
      -- 2. mutual recursion (cycle through another function)
      let mutual = [ g | g <- S.toList reach
                       , fName fd `S.member` reachable env g ]
      case mutual of
        (_:_) -> Left (EMutualRec (fName fd : mutual))
        []    -> Right ()
      -- 3. self-recursion: measure + tail
      let selfs = allSelfCalls (fName fd) (fBody fd)
      if null selfs then Right [] else do
        ...
        case findMeasure fd of
          ...
          Nothing -> Left (ENoMeasure (fName fd)
                 "no parameter with a finite lower-bound precondition \
                 \decreases syntactically (p - k, k>=1) at every self-call")
...
flipC :: COp -> COp
flipC CLt = CGt; flipC CLe = CGe; flipC CGt = CLt
flipC CGe = CLe; flipC CEq = CEq; flipC CNe = CNe
...
absEval env me aenv e st = case e of
  ...
  If c t f ->
    let (envT, envF) = refine c aenv
        st0 = evalCond c st
        (it, st1) = if anyReach envT then absEval env me envT t st0
                                     else (Bot, st0)
        (iff, st2) = if anyReach envF then absEval env me envF f st1
                                      else (Bot, st1)
      ...
      Just fd ->
        let step (i, (arg, Param _ pre)) (accSt :: ChkSt) =
              ...
              in if ia `subItv` pre
                   ...
                   else dyn (DynPre site f i)
                          (note (NDynPre site f i ia pre) s1)
            st1 = foldr step st
                    (reverse (zip [0..] (zip as (fParams fd))))
            st2 = if f == me then note (NInduction me) st1 else st1
        in (fPost fd, st2)
        -- KEY MOVE: the call's abstract result is the callee's DECLARED
        -- post.  This is sound regardless of whether the callee's post
        -- was proven statically, because an unproven post got a runtime
        -- check inserted -- if control returns, the post held.
    ...
    case M.lookup f env of
      ...
      Just fd ->
        let (ilo, st1) = absEval env me aenv lo st
            ...
            st6 = note (NFoldBound site
                    ("iterations <= hi - lo + 1; bound follows from the \
                     \range, proven once for Fold, not per use")) st5
...
simp c = case c of
  ...
  CMax xs ->
    let ys = nub (concatMap (flatX . simp) xs)
        ks = [ x | CN x <- ys ]
        rest = [ y | y <- ys, notCN y ]
        kept = rest ++ [ CN (maximum ks)
                       | not (null ks), null rest || maximum ks > 0 ]
        -- costs are nonnegative, so a 0 constant never wins a max
    in wrap kept (CN 0) CMax
  CDivC e 1 -> simp e
...
symOfE sub e = case e of
  ...
  Bin Sub a b -> (\x y -> add x (CMul [CN (-1), y]))
                   <$> symOfE sub a <*> symOfE sub b
  Bin Mul a b -> (\x y -> CMul [x, y]) <$> symOfE sub a <*> symOfE sub b
  _ -> Nothing
  where add x y = CAdd [x, y]
...
wcetAll env = M.fromList [ (nm, wcetFn S.empty fd) | (nm, fd) <- M.toList env ]
  where
    ...
    costE vis me sub e = case e of
      ...
      If (Cmp _ a b) t f ->
        CAdd [ CN 1, costE vis me sub a, costE vis me sub b
             , CMax [costE vis me sub t, costE vis me sub f] ]
      Let x a b ->
      ...
      Call _ g as
        ...
        | otherwise -> CAdd (CN 1 : map (costE vis me sub) as
                                 ++ [calleeCost vis sub g as])
      Fold _ lo hi g acc ->
        ...
        in CAdd [ costE vis me sub lo, costE vis me sub hi
                , costE vis me sub acc, CMul [iters, per] ]
    ...
    calleeCost vis sub g as = case M.lookup g env of
      ...
      Just gd ->
        let w = wcetFn vis gd
            m = M.fromList
                  (zip (map pName (fParams gd))
                       (map (\a -> fromMaybe (CO "?") (symOfE sub a)) as))
        in substC m w
    ...
    chk (site, g, as) = case M.lookup g env of
      ...
      Just gd ->
        let want = length (fParams gd)
            got  = length as
        in case fBody gd of
             _ | isFoldSite site (fBody fd) -> Right () -- Fold passes args implicitly
...
compile prog =
  ...
  in case arityErrs ++ safeErrs of
       ...
       [] ->
         let st = foldl (\s f -> checkFn env f s)
                        emptySt { stNotes = termNotes } prog
  ...
  where
    ...
    preGuard site g as = case M.lookup g env of
      ...
      Just gd ->
        let ps = fParams gd
            bad = [ (i, x, pPre p)
                  | (i, (x, p)) <- zip [0..] (zip as ps)
                  , DynPre site g i `S.member` dset
                  , not (x `member` pPre p) ]
        in case bad of
      ...
      | otherwise = case e of
        ...
        Bin op a b -> do
          ...
          r <- case op of
            ...
            Sub -> Right (x - y)
            Mul -> Right (x * y)
            Div -> if y == 0 then Left (ROther "div by zero")
                             else Right (x `div` y)
          Right (f2, r)
...
report prog c = unlines (concatMap perFn prog)
  where
    ...
    perFn fd =
      let nm = fName fd
          hdr = nm ++ " (" ++ intercalate ", " (map showParam (fParams fd))
                   ++ ") -> " ++ show (fPost fd)
                   ++ "   [" ++ (case fSafety fd of
                                   SafeFn -> "safe"; UnsafeFn -> "UNSAFE") ++ "]"
          wline = case M.lookup nm wmap of
            Nothing -> []
            Just w  -> ["  wcet: " ++ prettyC w ++ " ops"
                        ++ (if hasOmega w
                              then "   (\969 = opaque unsafe cost)" else "")]
          hasOmega x = case x of
            CO _ -> True
            CAdd xs -> any hasOmega xs; CMul xs -> any hasOmega xs
            CMax xs -> any hasOmega xs; CDivC e _ -> hasOmega e
            _ -> False
          mine nt = case nt of
          ...
          fmt nt = case nt of
            NTermination _ p k lo ->
              "  termination: measure " ++ p ++ " decreases by >= " ++ show k
              ++ " per self-call, precondition lower bound " ++ show lo
              ++ "  ==>  call count <= (" ++ p ++ " - " ++ show lo
              ++ ")/" ++ show k ++ " + 1   (static WCET shape)"
            NInduction _ ->
              "  self-call result taken from declared post (induction hypothesis)"
            NStaticPre site g i got need ->
              "  pre  @site " ++ show site ++ " " ++ g ++ "#arg" ++ show i
              ++ ": derived " ++ show got ++ " <= required " ++ show need
              ++ "   STATIC, no runtime check"
            NDynPre site g i got need ->
              "  pre  @site " ++ show site ++ " " ++ g ++ "#arg" ++ show i
              ++ ": derived " ++ show got ++ " NOT <= " ++ show need
              ++ "   ==> DYNAMIC check inserted"
            NPostProven _ got need ->
              "  post: derived " ++ show got ++ " <= declared " ++ show need
              ++ "   PROVEN statically"
            NPostDyn _ got need ->
              "  post: derived " ++ show got ++ " NOT <= declared " ++ show need
              ++ "   ==> DYNAMIC post check inserted"
            NFoldBound site d ->
              "  fold @site " ++ show site ++ ": " ++ d
      in (hdr : wline ++ map fmt (filter mine (cNotes c))) ++ [""]

showErr :: CompileError -> String
showErr = \case
  ENotTail f site ->
    "error[" ++ f ++ "]: self-call at site " ++ show site
    ++ " is NOT in tail position -- pending work per frame means the cost \
       \model is not `calls x per-call`; mark unsafe or rewrite with an \
       \accumulator / recursion scheme"
  ENoMeasure f why ->
    "error[" ++ f ++ "]: cannot certify termination: " ++ why
    ++ " -- mark unsafe or rewrite as a builtin recursion scheme (Fold)"
  ESafeCallsUnsafe f g ->
    "error[" ++ f ++ "]: declared safe but (transitively) calls UNSAFE `"
    ++ g ++ "` -- unsafety is infectious upward; mark " ++ f ++ " unsafe"
  EMutualRec fs ->
    "error: mutual recursion among safe functions " ++ show fs
    ++ " -- conservatively rejected (measure would need to be checked \
       \across the group)"
  EUnknownFn f g -> "error[" ++ f ++ "]: unknown function " ++ g
  EArity site g w g' ->
    "error: call at site " ++ show site ++ " to " ++ g ++ ": expected "
    ++ show w ++ " args, got " ++ show g'
...
mainProg =
  [ -- tail-recursive factorial with accumulator.
    ...
    FnDef "fact_acc" [Param "x" (atLeast 0), Param "acc" (atLeast 1)]
          (atLeast 1) SafeFn $
      If (Cmp CEq (v "x") (n 0))
         (v "acc")
  ...
  , FnDef "mul_step" [Param "i" (atLeast 1), Param "acc" (atLeast 1)]
          (atLeast 1) SafeFn $
      v "i" .*. v "acc"
  ...
  , FnDef "log_step" [Param "i" (atLeast 1), Param "acc" (atLeast 0)]
          (atLeast 0) UnsafeFn $
...
runCase c f as =
  ...
  where
    pretty (RPreViolation g i x need) =
      "dynamic PREcondition check: " ++ g ++ "#arg" ++ show i
      ++ " = " ++ show x ++ " not in " ++ show need
    pretty (RPostViolation g r need) =
      "dynamic POSTcondition check: " ++ g ++ " returned " ++ show r
      ++ " not in " ++ show need
    pretty RFuel = "fuel exhausted (unbounded recursion -- this is what \
                   \`unsafe` permits)"
    pretty (ROther s) = s
