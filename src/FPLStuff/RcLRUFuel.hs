...
-- ArcVal: handle to Arc.qa-owned shared memory
data Value = IntVal Integer | BoolVal Bool | ArcVal ArcId | UnitVal deriving (Show, Eq)
...
chargeFuel what cost = do
  ...
  if wFuel w < cost
    then do
      say $ "  [fuel]  exhausted (" ++ show (wFuel w) ++ " < " ++ show cost ++ " for " ++ what ++ ") -> yield()"
      ...
      say $ "  [fuel]  -" ++ show cost ++ " (" ++ what ++ "), remaining " ++ show (wFuel w - cost)
...
arcAlloc = do
  ...
  arcSend ("alloc #" ++ show aid ++ " (count=1, caller's ref)") (Map.insert aid 1)
  modify (\w' -> w' {wHeldArcs = aid : wHeldArcs w'})
  pure aid

arcIncref :: String -> ArcId -> M ()
arcIncref why aid = arcSend ("incref #" ++ show aid ++ " (" ++ why ++ ")") (Map.adjust (+ 1) aid)

arcDecref :: String -> ArcId -> M ()
arcDecref why aid = arcSend ("decref #" ++ show aid ++ " (" ++ why ++ ")") (Map.adjust (subtract 1) aid)
...
callFn defs fname argVals = do
  ...
  case (fnTabled fn, traverse asInt argVals) of
    ...
    (True, Just intArgs) -> do
      ...
      say $ "  [hash]  h" ++ show intArgs ++ " = (sum a_i*x_i) >> " ++ show (64 - tK t) ++ " = slot " ++ show idx0
      now <- tickUp
      -- 2. linear probe: match -> HIT, empty -> insert slot, else remember LRU victim
      case probe t idx0 intArgs of
        PHit slot entry -> do
          say $ "  [table] HIT at slot " ++ show slot ++ " -> " ++ show (eVal entry) ++ "  (precond + body SKIPPED)"
        ...
        PMiss whereTo -> do
          ...
          slot <- case whereTo of
            ...
            EvictLRU s victim -> do
              say $ "  [table] full: evict LRU slot " ++ show s ++ " key=" ++ show (eKey victim)
              case eVal victim of
...
data ProbeResult = PHit Int Entry | PMiss MissSlot

data MissSlot = EmptySlot Int | EvictLRU Int Entry
    ...
    go i firstEmpty
      | i == n =
          case firstEmpty of
            ...
            Nothing ->
              let occupied = [(s, e) | s <- [0 .. n - 1], Just e <- [Map.lookup s (tSlots t)]]
...
putEntry f s e = modify (\w -> w {wTables = Map.adjust (\t -> t {tSlots = Map.insert s e (tSlots t)}) f (wTables w)})
...
crashCleanup reason = do
  ...
  let tableArcs = [aid | t <- Map.elems (wTables w), e <- Map.elems (tSlots t), ArcVal aid <- [eVal e]]
...
defs =
  Map.fromList
    [ ( "square",
      ...
      ( "mkShared",
        FnDef -- returns Arc.qa-backed memory
          { fnParams = ["n"],
            fnPrecond = Nothing,
...
initialWorld = World {wFuel = 14, wBudget = 14, wTick = 0, wTables = Map.fromList [("square", newTable 4 2 [0x9E3779B97F4A7C15, 0xC2B2AE3D27D4EB4F]), ("mkShared", newTable 4 2 [0xFF51AFD7ED558CCD, 0xC4CEB9FE1A85EC53])], wHeldArcs = [], wArcQa = Map.empty, wNextArc = 0, wLog = []}
