...
data Expr
  ...
  | Lit    Ty               -- literal (always Many)
  | Pipe   Expr Expr        -- smart |> threading
  | PipeFn String Ty Ty     -- named function: name, argTy, retTy
  | With   String Ty Expr   -- with varName : ty { pipeline }
  | IfExpr Expr Expr Expr   -- if _ then e1 else e2
  deriving (Show)
...
checkConsumed names env = mapM_ go names
  where
    go name = case Map.lookup name env of
      ...
      Just (_, One, n) | n > 1
                       -> Left $ "Linear variable '" ++ name ++ "' used " ++ show n ++ " times"
...
main = do
  ...
  -- 1. Simple pipeline
  run "bytes |> json.parse  =>  Result Value"
  ...
  run "(File 1, Result Bytes) |> json.parse  =>  (File 1, Value)"
    (declare "fp" (tLinearPair tBytes) One emptyEnv)
    (Pipe (Var "fp") (PipeFn "json.parse" tBytes tValue))
  ...
  run "with f { f |> read |> json.parse }  =>  OK"
    emptyEnv
    (With "f" tFileLinear
      (Pipe
        (Pipe (Var "f")
  ...
  run "with f { bytes |> json.parse }  =>  f unconsumed, FAIL"
    (declare "bytes" tBytes Many emptyEnv)
    (With "f" tFileLinear
      (Pipe (Var "bytes") (PipeFn "json.parse" tBytes (TResult tValue))))

  -- 5. Linear var used twice  =>  FAIL
  run "f |> close, then f again  =>  used twice, FAIL"
  ...
  runU "File 1  ~  File  (consuming linear, OK)"
    tFileLinear tFile

  -- 7. Unification: File with Bytes  =>  FAILG
  runU "File  ~  Bytes  (should fail)"
    tFile tBytes

  -- 8. Unification with TVar: (File 1, Result a) ~ (File 1, Result Bytes)
  runU "(File 1, Result a)  ~  (File 1, Result Bytes)  =>  a := Bytes"
    (tLinearPair (TVar "a")) (tLinearPair tBytes)

  -- 9. IfExpr: both branches consume f  =>  OK
  run "if _ then close f else close f  =>  OK"
    (declare "f" tFileLinear One emptyEnv)
    (IfExpr
      (Lit tBool)
      (Pipe (Var "f") (PipeFn "close" tFileLinear tUnit))
  ...
  run "if _ then close f else ()  =>  else forgets f, FAIL"
    (declare "f" tFileLinear One emptyEnv)
    (IfExpr
      (Lit tBool)
      (Pipe (Var "f") (PipeFn "close" tFileLinear tUnit))
