...
burialEdges js =
  [ (jId user, jId burier)
  | burier <- js, user <- js, jId burier /= jId user
...
componentBBoxAfter lib comps Joint{..} =
  let boxes = [ maybe (bboxOf . pKind $ lib M.! p) snd (M.lookup p comps)
              | p <- jParts ]
           ...
           _  ->
             let scored = sortOn score ready
                 ...
                 area    = case filter (fits (w, d)) areas of
                             (a:_) -> aId a
                             []    -> "!! NO AREA FITS !!"
             in go (filter ((/= jId j) . jId) pending)
...
lib = M.fromList [ (pId p, p) | p <- ps ]
  where
    ps = [ Part "ext-A1" (Extrusion2020 600), Part "ext-A2" (Extrusion2020 600)
         , Part "ext-B1" (Extrusion2020 600), Part "ext-B2" (Extrusion2020 600)
         , Part "cb-1" (CornerBracket 90), Part "cb-2" (CornerBracket 90)
         , Part "cb-3" (CornerBracket 90)
         , Part "pb-1" PanelBracket, Part "pb-2" PanelBracket
         , Part "panel-1" (Panel 600 600), Part "panel-2" (Panel 600 600)
         , Part "leg-1" (Extrusion2020 700), Part "leg-2" (Extrusion2020 700)
         , Part "leg-3" (Extrusion2020 700), Part "leg-4" (Extrusion2020 700)
         ]
...
joints =
  ...
  , Joint "J3-frame" ["ext-A1","ext-B1","cb-3"]
      ...
      True
  , Joint "J4-panels" ["ext-A1","panel-1","panel-2","pb-1","pb-2"]
      ...
      True
  , Joint "J5-square" ["ext-A1"]
      [topFace "ext-A1"] []                      -- needs the face J4 buries
      ["J3-frame"]
      "Straightedge squareness check across bare frame top rails; shim/adjust corners."
      True
  , Joint "J6-legs" ["ext-A1","leg-1","leg-2","leg-3","leg-4"]
      [] []
      ["J4-panels"]
...
main = do
  ...
  case planOrder lib areas joints of
    ...
    Right steps -> do
      ...
      putStrLn $ "  Final bbox " ++ showV3 deskWithLegs ++ " vs " ++ dwId door
               ++ " (" ++ show (dwW door) ++ "x" ++ show (dwH door) ++ "): "
               ++ (if deskWithLegs `passes` door then "PASSES" else "BLOCKED")
      mapM_ (putStrLn . ("  " ++)) errs
      mapM_ (\j -> putStrLn $ "  -> Defer joint " ++ j
                           ++ " until after transport (frame-only bbox "
                           ++ showV3 deskWithoutLegs ++ " passes).") defer
  where
    render (n, Placed{..}) = putStrLn $
      "  Step " ++ show n ++ " [" ++ plArea ++ ", bbox " ++ showV3 plBox ++ "]"
