...
checkNets comps nets k
  ...
  | otherwise = k
  where
    bad (_, prs) = length (nub (map (domainOf comps) prs)) > 1
...
solveStep ::
  [Component] ->
  [(String, [PortRef])] ->
  M.Map CompName State ->
  Double ->
  (M.Map String Double, M.Map CompName Solved)
solveStep comps nets states dt = (netVals, solvedByComp)
  where
    ...
    ixFor cn pn =
      ...
      where
        err w = error ("PortSim: unknown " ++ w ++ " for " ++ cn ++ "." ++ pn)
...
gauss n eqns
  ...
  where
    rows0 =
      [ [M.findWithDefault 0 j m | j <- [0 .. n - 1]] ++ [rhs]
        | (terms, rhs) <- eqns,
...
velSource nm dom w =
  ...
  where
    eqs _ _ ix = let (a, _) = ix "p" in [([(a, 1)], w)]
...
effSource nm dom f =
  Component nm [("p", dom)] eqs M.empty stateless
  where
    eqs _ _ ix = let (_, t) = ix "p" in [([(t, 1)], -f)]
...
groundC nm dom =
  Component nm [("p", dom)] eqs M.empty stateless
  where
    eqs _ _ ix = let (a, _) = ix "p" in [([(a, 1)], 0)]
...
exGears = do
  ...
  simulate sys 1 1 $ \_ _ nv sv _ -> do
    ...
    printf
      "  speeds   : in %.3f  mid %.3f  out %.4f rad/s  (expected out = 10/12 = 0.8333)\n"
      (w "shaftIn")
      (w "shaftMid")
      (w "shaftOut")
    printf
      "  torques  : motor supplies %.4f Nm, load absorbs %.4f Nm (12x amplification)\n"
      (negate tMotor)
      tLoad
    printf
      "  power    : motor %.4f W  ==  load %.4f W (lossless transmission)\n"
      (negate tMotor * w "shaftIn")
      (tLoad * w "shaftOut")
